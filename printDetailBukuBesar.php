...
<head>
	...
	<title>Rincian Buku Besar Id Rekening : <?php echo $_REQUEST['id_rek']; ?></title>
...
<body onload="prin()">
	<center>
		...
		<?php }else{ ?>
		<h4>Rincian Buku Besar <?php
		$qq = mysql_query("SELECT * FROM rekening WHERE id_rek = '$_REQUEST[id_rek]'");
		$rr = mysql_fetch_array($qq);
		echo $rr[1];
		?><br>Periode <?php echo date('d-m-Y',strtotime($_REQUEST['dari'])); ?> Sampai <?php echo date('d-m-Y',strtotime($_REQUEST['ke'])); ?></h4>
		<?php } ?>
	</center>
	<?php
		$no = 1;
		$databesar = mysql_query("SELECT * FROM rekening WHERE id_rek = '$_REQUEST[id_rek]'");
		$rek = mysql_fetch_array($databesar); ?>
